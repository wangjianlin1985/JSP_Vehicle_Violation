...
<%
	...
	if (action == null || "".equals(action) || uid == null
			|| "".equals(uid)) {
		return;
		...
		if ("".equals(uid) || uid.length() <= 0) {
		...
		} else if (!"".equals(ParamUtils.nullToString(bf.SearchUser(
				"where uid='" + uid + "'").getUid()))) {
			...
			session.setAttribute("error", "恭喜你，注册成功！");
			response.sendRedirect("error.jsp");
		} else {
		...
		}else if (uid != null && !"".equals(uid) && u.getStatus() == FinalConstants.STATUS_USER_T) {
			session.setAttribute("user", u);
			response.sendRedirect("admin/index.jsp");
		}else if (uid != null && !"".equals(uid) && u.getStatus() == FinalConstants.STATUS_USER_TT) {
			session.setAttribute("user", u);
		...
		String reganswer = ParamUtils.getRequestString(request,
				"reganswer");
		...
		if (!"".equals(uid) && "".equals(regqu)) {
			...
			RequestDispatcher rd = request
					.getRequestDispatcher("user_return.jsp");
			rd.forward(request, response);
		} else if (reganswer != null && !"".equals(reganswer)
				&& regqu != null && !"".equals(regqu)) {
			User u = bf.SearchUser("WHERE reganswer='" + reganswer
					+ "' AND uid='" + uid + "' AND regqu='" + regqu
					+ "'");
		...
		if (u == null || "".equals(u.getUid())) {
			...
			u.setAddress(ParamUtils
					.getRequestString(request, "address"));
